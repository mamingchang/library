...
public class administaror_fine extends JPanel implements ActionListener
{
	...
	public administaror_fine(String new_id)
	{
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if(e.getSource()==jb1)
		{//事件源为"查询欠款"按钮
			String find = "Select money from fine where id = '"+this.administaror_id+"'";
		...
		{//事件源为"交费"按钮
			...
			else
			{
				...
				try {
					...
					{
						statement.execute("update fine set money = money-"+answer2+" where id = '"+this.administaror_id+"'");
					}
					else
					{
						statement.execute("update fine set money = 0 where id = '"+this.administaror_id+"'");
					}
					result=statement.executeQuery(find);
					if(result.next())
					{
					...
					if(answer2 == 0)
					{
						statement.execute("update administraor set permitted = '是' where id = '"+this.administaror_id+"'");
					}
				} catch (Exception e2) {
					e2.printStackTrace();
				}
