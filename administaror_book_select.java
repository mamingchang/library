...
public class administaror_book_select extends JPanel implements ActionListener
{
	int flag;
	String sql;
	DataBase db;
   //创建分割方向为上下的JSplitePane对象
	...
	public administaror_book_select(){
	        	...
	        	if(jcb.getSelectedIndex()>=0&&jcb.getSelectedIndex()<4){
				    ...
				    if(e.getSource()==jb){//如果事件源为"提交"按钮，则执行检索
					    ...
					    if(jcb.getSelectedIndex()==0){//根据书名进行查询
							sql="select * from book_info where name='"+str+"'";
				        	jtxtArray[3].setText("");
						}
						else if(jcb.getSelectedIndex()==1){//根据出版社进行查询
							sql="select * from book_info where press='"+str+"'";
							jtxtArray[3].setText("");
						}
						else if(jcb.getSelectedIndex()==2){//根据作者进行查询
							sql="select * from book_info where author='"+str+"'";
							jtxtArray[3].setText("");
						}
						else{//根据购进时间进行查询
							sql="select * from book_info where time='"+str+"'";
							jtxtArray[3].setText("");
						}
					//db=new DataBase();;
//尝试性选择-开始
