...
public class student_book_return extends JPanel implements ActionListener{
	DataBase db;
	String sql;
	String str;
	...
	public student_book_return(String new_id)
	{
		this.student_id = new_id;
		this.data = first_data(this.data,this.student_id);
		//创建表格模型
 ...
 //尝试性删除-开始
   		...
   		try {
   				Class.forName("com.mysql.cj.jdbc.Driver");
   				 connect  = DriverManager.getConnection("jdbc:mysql://localhost/library?serverTimezone=GMT", "root", "123");
   				 statement = connect.createStatement();
   				 statement.execute("delete from record where id = '"+this.student_id+"' and book_name = '"+selected_book_name+"'");
   				}
   				catch(Exception ex){ex.printStackTrace();}
   				finally
   				{
   		...
   		try {
   			Class.forName("com.mysql.cj.jdbc.Driver");
   			 connect  = DriverManager.getConnection("jdbc:mysql://localhost/library?serverTimezone=GMT", "root", "123");
   			 statement = connect.createStatement();
   			result = statement.executeQuery("select * from record where id = '"+this.student_id+"'");
   			data = new Vector<Vector>();
   			 while(result.next())
   			 {
   				 accept = new Vector<>();
   		...
   		//尝试性删除-结束
   			...
   			{//当对借阅的图书进行操作时
   				...
   				Vector<String>accept = null;
   				String gengxincaozuo = "update fine set money = money+"+cha+" where id = '"+this.student_id+"'";
   				//更新用户的罚款表
   				String genggaiquanxian = "update student set permit = '否' where xuehao = '"+this.student_id+"'";
   				//更改用户的权限为否
...
//注意此处是要更改的查询是否有高级权限的用户有预约的信息时
   		...
   		{//需要挂失图书
			...
			else
			{
				...
				String sql2="update fine set money = money+1000 where id = '"+this.student_id+"'";
				String sql3="update student set permit = '否'  where xuehao = '"+this.student_id+"'";
				//尝试性选择-开始
				Connection connect = null;
				Statement statement  = null;
				ResultSet result = null;
...
//尝试性选择-结束
   		...
   		if(e.getSource().equals(jbArray[3]))
   		{
   			Connection connect = null;
			Statement statement  = null;
			ResultSet result = null;
